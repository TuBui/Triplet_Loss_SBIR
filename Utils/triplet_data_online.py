...
same as triplet_data_layer.py
mix pos and neg together, also return labels
@author: tb00083
"""
...
class TripletDataLayer(caffe.Layer):
  ...
  def forward(self,bottom,top):
    ...
    top[1].data[...] = np.concatenate((res['data_p'],res['data_n']),axis=0)#.astype(np.float32,copy = True)
    top[2].data[...] = res['label_a']#.astype(np.float32,copy = True)
    top[3].data[...] = np.concatenate((res['label_a'],res['label_n']),axis=0)#.astype(np.float32,copy=True)
    #3
...
class BatchLoader(object):
  ...
  def load_next_batch(self):
    ...
    res['label_a'] = np.asarray([tri['label_a'] for tri in thread_res],dtype=np.float32)
    res['label_n'] = np.asarray([tri['label_n'] for tri in thread_res],dtype=np.float32)
    return res
#==============================================================================
