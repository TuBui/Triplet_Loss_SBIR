...
same as triplet_data.py, here work with hardsel
@author: tb00083
"""
...
class TripletDataLayer(caffe.Layer):
  ...
  def forward(self,bottom,top):
    ...
    Load data.
    """
    if self.batch_loader_refresh:
      self.batch_loader.update_hardsel_table()
    ...
    #3
    if self.batch_loader.IsEpochEnded():
      self.batch_loader_refresh = True
    else:
      self.thread_results = self.pool.apply_async(self.batch_loader.load_next_batch, ())
...
class BatchLoader(object):
  ...
  def __init__(self, params):
    ...
    self.watchChange = WatchDog(self.hard_sel_file) #check if file has been updated
  ...
  def update_hardsel_table(self):
    """update hard selection list"""
    assert self.watchChange.IsUpdated(),'Opps! File hasnt changed since last read'
    self.hardsel_tab = self.hard_pos = self.hard_neg = None
    self.hardsel_tab = np.load(self.hard_sel_file)
    self.hard_pos = self.hardsel_tab['pos']
    self.hard_neg = self.hardsel_tab['neg']
  ...
  def load_next_triplet(self,l):
    ...
    l.acquire() #5
#==============================================================================
#     if self._cur == self.NSKTS:
#       self._cur = 0
#       self.shuffle_keeping_min_classes_per_batch()
#       self.update_hardsel_table()
#==============================================================================
    ...
    if self.hard_sel == 'off':    #hard selection turned off
    ...
    elif self.hard_sel == 'hard_pos':  #hard positive selection
      index_p = np.random.choice(self.hard_pos[index])
      label_n = label
      while label_n == label:
        label_n = np.random.choice(self.classes)
      index_n = np.random.choice(self.img_labels_dict[str(label_n)])
    elif self.hard_sel == 'hard_neg':  #hard neg
      index_p = np.random.choice(self.img_labels_dict[str(label)])
      index_n = np.random.choice(self.hard_neg[index])
    elif self.hard_sel == 'hard_pn':   #hard pos and neg 'hard_pn'
      index_p = np.random.choice(self.hard_pos[index])
      index_n = np.random.choice(self.hard_neg[index])
    else: #should never happens
      assert 1==0,'hard selection invalid'
...
class WatchDog(object):
    ...
    def IsUpdated(self):
      """always return True when called 1st time"""
